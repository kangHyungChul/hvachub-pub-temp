const setRole = document.querySelector(".setting-role");
const setPerm = document.querySelector(".setting-permission");
const nextBtn = setRole.querySelector(".setting-btn-wrap .btn-block");

nextBtn.addEventListener("click", function () {
  setRole.classList.remove("is-show");
  setPerm.classList.add("is-show");
});

// 드래그 가로스크롤
const scrollContainer = document.querySelectorAll(".pas-result-wrap .list-check");
...
function rowHeightChange() {
  const roleLists = document.querySelectorAll(".pas-result-wrap > .role-list");
  roleLists.forEach((list) => {
    const listMenu = list.querySelectorAll(".list-menu .list__item");
...
function updateBoxHeight(elements, maxHeight) {
  elements.forEach((element) => {
    ...
    if (windowWidth >= 1440) {
      ...
      element.style.minHeight = `${vwHeight}vw`;
    } else {
      element.style.minHeight = `${maxHeight}px`;
    }
  });
