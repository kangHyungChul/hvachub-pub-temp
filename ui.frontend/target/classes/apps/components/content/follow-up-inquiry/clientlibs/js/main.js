const followUpInquiry = (function () {

  // file list 노출 여부에 따른 margin 설정
  const fileListMargin = function () {
    const $fileListWrap = $('.follow-up-inquiry .file-list__wrap');
    if($('.follow-up-inquiry .file-list__el').length > 0){
  ...
  const fildeNameEllipsis = function(){
    $('.follow-up-inquiry .file-list__el .file-name').each(function(){
...
$(document).ready(function () {
  followUpInquiry.init();

  $(window).on('resize', function(){
    followUpInquiry.fileListMargin();
  })
});